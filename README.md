...
Hard-reload once if you still see the old game (the PWA can keep a cached copy). Live bundle is published with V0 **1.4.70**. **Easy mode** is off by default — check **Easier words · bigger taps** on Welcome, or Settings → Reading → Easy mode. Easy home is Match → Hold on the Easy trail (all 39 facts in pack order); Mercy is a collect-help road maze with Helped! then Hold next and One more road; gem finds flip comic story panels, extra words on the board (a fresh 4–6 from the 3–6 letter dictionary each Match) score **BONUS! +100**; a miss swipe is **Miss −25** (floor 0); Easy Hold why-blast uses four chips and **Miss −25**; the father-run is a hold-to-run timing dash with Hugged! then Hold next and One more run; Witness Square’s creed is a Suika-style candy merge bowl with a loud Drop ghost, smash/combo/Miss pops, and MERGED! then one takeaway then Hold next. Cold Easy starts at Mercy / Story Creek until that line is held on Easy — Hard or older saves do not skip it. After the Story Creek opening, Why Gate’s foundation pack unlocks in order (order → reason → ought → living God). After that Hold, Witness Square Dig deeper is ancient-first arcade: creed merge, then women / early / method tap-ins (glowing tablets, DUG!, Miss −25). Easy Dig deeper is Dig the names — two or three scripture/ancient taps, not essay homework. Learn stays as an unboxed re-read. Town (soon) waits under Settings → More. Night Watch stays on Hard until Match→Hold is solid. We teach hard words first: a claim is the main idea we hold to be true. After that, Easy buttons say main idea. After a win, say the line out loud; Dig deeper holds the reason. Open **Profile** from Town or Settings for every unlock. **Support the trail** (Settings or Easy home) opens Packs: the core Easy trail stays free; extra streets (Mill Street, Harbor Walk) are optional unlocks; Remove ads is a supporter gift. On GitHub Pages that unlock is an honest this-device stub plus Restore — the same flags Play Billing will set later. Soft pauses sit between home and a lesson only, never over Match, Hold, Journal, or arcade play, and never as a hard paywall.
...
- **UI:** Hub is the **overworld town** on Hard. Completing Daily / districts / journal / stars lights buildings you **manage and upgrade** by tapping. Tap a place to open its **manage** sheet (person + idea nodes). **Profile** (Town tools or Settings) lists River plus every unlock. **Dig deeper** on Hold, Journal, mind map, and Profile: Scripture and older witnesses when they fit; later faithful sources welcome when they help. Settings shows **What’s new** plus schema and app version (`1.4.70`). **Easy mode** (Welcome or Settings → Reading) hides the Town map: home is Match → Hold on the Easy trail (39 pack facts, starting with Mercy at Story Creek; after the Story Creek opening a Why Gate foundation pack unlocks in order; after Why Gate, Witness Square Dig deeper is creed merge then women/early/method tap-ins; Mercy is a collect-help road maze; gem finds flip story panels; extra board words from the 3–6 letter dictionary score +100 (a miss is −25); Easy Hold why-blast is four chips with Miss −25; father-run is a timing dash with One more run; the creed at Witness Square is a candy merge bowl), and Town (soon) waits under Settings → More. Night Watch stays on Hard. Easy teaches hard words first (a claim is the main idea we hold to be true) and then uses main idea in buttons. After every win you say the line out loud. Reset progress lives only in Settings → Danger zone. **Support the trail** / Remove ads / Packs live at the top of Settings (and on Easy home). Core Easy stays free; extra streets are optional packs.
...
- **Version:** app `1.4.70` V0 (`package.json`, `src/config/app.ts`, Android `versionName`). Capacitor id `city.silver.unending`. `npm run android:apk` still builds the debug APK. Settings → **What’s new** (`src/content/changelog.ts`) is the hook for later packs. Learn → Match → Hold copy loads from `src/content/packs/` (schemaVersion 3 XML).
