...
- **Version:** app `1.4.76` V0 (`package.json`, `src/config/app.ts`, Android `versionName`). Capacitor id `city.silver.unending`. `npm run android:apk` still builds the debug APK. Settings → **What’s new** (`src/content/changelog.ts`) is the hook for later packs. Learn → Match → Hold copy loads from `src/content/packs/` (schemaVersion 3 XML).
