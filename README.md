...
Hard-reload once if you still see the old game (the PWA can keep a cached copy). Live bundle is published with V0 **1.4.85**. **Easy mode** is off by default — check **Easier words · bigger taps** on Welcome, or Settings → Reading → Easy mode. Easy home is Match → Hold on the Easy trail (all 45 facts in pack order); Mercy is a collect-help road maze with Gemini road stills and Helped! then Hold next and One more road; gem finds flip comic story panels, extra words on the board (a fresh 4–6 from the 3–6 letter dictionary each Match) score **BONUS! +100**; after MATCHED, Hold next then gold One more Match on every win; a miss swipe is **Miss −25** (floor 0); Easy Hold why-blast uses four chips and **Miss −25**; the father-run is a hold-to-run timing dash with five Gemini stills, Hugged! then Hold next and One more run; Witness Square’s creed is a Suika-style candy merge bowl with a loud Drop ghost, smash/combo/Miss pops, and MERGED! then one takeaway then Hold next. Cold Easy starts at Mercy / Story Creek until that line is held on Easy — Hard or older saves do not skip it. After the Story Creek opening, Why Gate’s foundation pack unlocks in order (order → reason → ought → living God). After that Hold, Witness Square Dig deeper is ancient-first arcade: creed merge, then women / early / method tap-ins (glowing tablets, DUG!, then Hold next and One more dig, Miss −25). After Dig deeper Hold, Names that stay opens next: named people, churches already named Christ, then the empty tomb — same source-dig tap-ins, stepped after each Hold. After the Easy door, Stone Court opens: Tacitus, Josephus on James, then Pliny — ancient-first source-digs, stepped after each Hold. After Pliny Hold, Ink Court opens: Trajan, Suetonius, Lucian — the same ancient-first source-digs. Easy Dig deeper is Dig the names — two or three scripture/ancient taps, not essay homework. Learn stays as an unboxed re-read. Town (soon) waits under Settings → More. Night Watch stays on Hard until Match→Hold is solid. We teach hard words first: a claim is the main idea we hold to be true. After that, Easy buttons say main idea. After a win, say the claim · reason · source. Open **Profile** from Town or Settings for every unlock. **Support the trail** (Settings or Easy home) opens Packs: the core Easy trail stays free; extra streets (Mill Street, Harbor Walk) are optional unlocks; Remove ads is a supporter gift. On GitHub Pages that unlock is an honest this-device stub plus Restore — the same flags Play / App Store IAP will set when the adapter has a plugin and `VITE_PLAY_BILLING` / `VITE_STOREKIT`. Soft pauses sit between home and a lesson only, never over Match, Hold, Journal, or arcade play, and never as a hard paywall. Capacitor **ios** + **android** shells share this codebase; store money still waits on the one-time connect below.
...
| Why Gate | Foundation (order, reason, ought, living God), then first mover, contingency, kalām — and what those arguments do *not* yet prove |
...
- **UI:** Hub is the **overworld town** on Hard. Completing Daily / districts / journal / stars lights buildings you **manage and upgrade** by tapping. Tap a place to open its **manage** sheet (person + idea nodes). **Profile** (Town tools or Settings) lists River plus every unlock. **Dig deeper** on Hold, Journal, mind map, and Profile: Scripture and older witnesses when they fit; later faithful sources welcome when they help. Settings shows **What’s new** plus schema and app version (`1.4.85`). **Easy mode** (Welcome or Settings → Reading) hides the Town map: home is Match → Hold on the Easy trail (45 pack facts, starting with Mercy at Story Creek; after the Story Creek opening a Why Gate foundation pack unlocks in order; after Why Gate, Witness Square Dig deeper is creed merge then women/early/method tap-ins with One more dig; after Dig deeper Hold, Names that stay is names/creed-close/empty source-digs; after the Easy door, Stone Court is Tacitus/James/Pliny ancient-first source-digs; after Pliny Hold, Ink Court is Trajan/Suetonius/Lucian ancient-first source-digs; Mercy is a collect-help road maze with Gemini road stills; gem finds flip story panels; extra board words from the 3–6 letter dictionary score +100 (a miss is −25); after MATCHED, One more Match is always the gold replay; Easy Hold why-blast is four chips with Miss −25; father-run is a timing dash with five Gemini stills and One more run; the creed at Witness Square is a candy merge bowl), and Town (soon) waits under Settings → More. Night Watch stays on Hard. Easy teaches hard words first (a claim is the main idea we hold to be true) and then uses main idea in buttons. Easy held lines keep claim · reason · source to say tomorrow. Reset progress lives only in Settings → Danger zone. **Support the trail** / Remove ads / Packs live at the top of Settings (and on Easy home). Core Easy stays free; extra streets are optional packs.
## The town
...
No live ad network in this build. Between-scene pauses are labeled, dismissible, in-app copy.

- **Network flag:** `adsEnabledDefault` in `src/config/ads.ts` is **false**. Do not load a vendor SDK until a real unit is wired.
- **Soft pauses:** unpaid walks may see a dismissible pause **between home and a lesson** (`hub` ↔ Learn / Match / Daily / a district walk / an extra street). They mount on home, never over Match, Hold, Journal, or arcade play (claim-merge, panel-blast, father-run, road-maze, source-dig).
- **Remove ads:** Settings → Support the trail (SKU `city.silver.unending.removeads`). On Pages this is an honest this-device stub + Restore. Play Billing should call the same `grantRemoveAds()` later.
- **Must never be blocked or covered:** Keep/Toss; takeaway / RecallGate; Journal due cards and held evidence; Match boards; Hold Why Blast; arcade play. See `ADS_NEVER_COVER`.

## Support / packs

- **Core Easy trail stays free.** Never a hard paywall. Never gate held Journal.
- **Packs shop:** `src/config/commerce.ts` — Mill Street and Harbor Walk are extra streets behind local unlock flags (`grantPack`). Same flags are what Play IAP will set (`city.silver.unending.pack.mill` / `.harbor`).
- **Web:** Pages cannot charge. Shop copy says so. Unlock on this device exercises the gates. Do not inject a checkout script.

## Support

- **Version:** app `1.4.85` V0 (`package.json`, `src/config/app.ts`, Android `versionName`). Capacitor id `city.silver.unending`. `npm run android:apk` still builds the debug APK. Settings → **What’s new** (`src/content/changelog.ts`) is the hook for later packs. Learn → Match → Hold copy loads from `src/content/packs/` (schemaVersion 3 XML).
- **Content packs:** core trail is `CONTENT_PACKS` in [`src/content/packs.ts`](src/content/packs.ts). Paid extra streets are shop SKUs in [`src/config/commerce.ts`](src/config/commerce.ts), not a lock on the 45-fact Easy trail. Next harvest drop: new Area module → register in `index.ts` → one pack row → changelog row → journal/evidence/city/tools. Details: [`src/content/README.md`](src/content/README.md).- **Release smoke:** [`PLAYTEST.md`](PLAYTEST.md) (fun / clarity / retention plus the short checklist).
