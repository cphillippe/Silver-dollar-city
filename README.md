...
Hard-reload once if you still see the old game (the PWA can keep a cached copy). Live bundle is `index-BcheE80F.js` — V0 **1.3.3**; open **Profile** from Town or Settings for every unlock.
...
- **Version:** app `1.3.3` V0 (`package.json`, `src/config/app.ts`, Android `versionName`). Capacitor id `city.silver.unending`. `npm run android:apk` still builds the debug APK. Settings → **What’s new** (`src/content/changelog.ts`) is the hook for later packs.
