...
- **UI:** Hub is the **overworld town** on Hard. Completing Daily / districts / journal / stars lights buildings you **manage and upgrade** by tapping. Tap a place to open its **manage** sheet (person + idea nodes). **Profile** (Town tools or Settings) lists River plus every unlock. **Dig deeper** on Hold, Journal, mind map, and Profile: Scripture and older witnesses when they fit; later faithful sources welcome when they help. Settings shows **What’s new** plus schema and app version (`1.4.77`). **Easy mode** (Welcome or Settings → Reading) hides the Town map: home is Match → Hold on the Easy trail (39 pack facts, starting with Mercy at Story Creek; after the Story Creek opening a Why Gate foundation pack unlocks in order; after Why Gate, Witness Square Dig deeper is creed merge then women/early/method tap-ins with One more dig; after Dig deeper Hold, Names that stay is names/creed-close/empty source-digs; Mercy is a collect-help road maze; gem finds flip story panels; extra board words from the 3–6 letter dictionary score +100 (a miss is −25); after MATCHED, One more Match is always the gold replay; Easy Hold why-blast is four chips with Miss −25; father-run is a timing dash with One more run; the creed at Witness Square is a candy merge bowl), and Town (soon) waits under Settings → More. Night Watch stays on Hard. Easy teaches hard words first (a claim is the main idea we hold to be true) and then uses main idea in buttons. Easy held lines keep claim · reason · source to say tomorrow. Reset progress lives only in Settings → Danger zone. **Support the trail** / Remove ads / Packs live at the top of Settings (and on Easy home). Core Easy stays free; extra streets are optional packs.
...
- **Version:** app `1.4.77` V0 (`package.json`, `src/config/app.ts`, Android `versionName`). Capacitor id `city.silver.unending`. `npm run android:apk` still builds the debug APK. Settings → **What’s new** (`src/content/changelog.ts`) is the hook for later packs. Learn → Match → Hold copy loads from `src/content/packs/` (schemaVersion 3 XML).
- **Content packs:** core trail is `CONTENT_PACKS` in [`src/content/packs.ts`](src/content/packs.ts). Paid extra streets are shop SKUs in [`src/config/commerce.ts`](src/config/commerce.ts), not a lock on the 39-fact Easy trail. Next harvest drop: new Area module → register in `index.ts` → one pack row → changelog row → journal/evidence/city/tools. Details: [`src/content/README.md`](src/content/README.md).
