...
Hard-reload once if you still see the old game (the PWA can keep a cached copy). Live bundle is published with V0 **1.3.5**. **Easy mode** is off by default — check **Easier words · bigger taps** on Welcome, or Settings → Reading → Easy mode. After a win, say the claim out loud; Dig deeper holds the reason. Open **Profile** from Town or Settings for every unlock.
...
- **UI:** Hub is the **overworld town**. Completing Daily / districts / journal / stars lights buildings. Tap a place to open its **mind map** (person + idea nodes). **Profile** (Town tools or Settings) lists River plus every unlock. **Dig deeper** on Hold, Journal, mind map, and Profile opens Scripture/Fathers first. Settings shows **What’s new** plus schema and app version (`1.3.5`). **Easy mode** (Welcome checkbox or Settings → Reading) uses shorter sentences and bigger taps; claims stay the same. After every win you say the claim out loud. Reset progress lives only in Settings → Danger zone.
...
- **Version:** app `1.3.5` V0 (`package.json`, `src/config/app.ts`, Android `versionName`). Capacitor id `city.silver.unending`. `npm run android:apk` still builds the debug APK. Settings → **What’s new** (`src/content/changelog.ts`) is the hook for later packs.
