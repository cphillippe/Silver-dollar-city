...
export const CHANGELOG: ChangeNote[] = [
  ...
  },
  {
    ...
    when: '2026-09-18',
    items: [
      'Road maze win is clean Helped! (no double frame / yellow mash); Inn/Hurt named once on the beat strip',
    ],
  },
  {
    version: '1.4.91',
    title: 'Fun Match pin story strip · Easy core trail',
    when: '2026-09-18',
    items: [
      'Match pins story pictures while the crossword scrolls',
    ],
  },
  {
    version: '1.4.90',
  ...
  },
  {
    ...
    when: '2026-09-16',
    items: [
      'Easy held lines stay a kid-plain claim · reason · source. After MATCHED, DUG!, MERGED!, Helped!, Hugged!, and Hold LOCKED!, the win card names why and From — one breath, not a quiz',
      'Leftover hedges drop: Meaning is received, not only built. Something is here — it did not have to be. Jesus is the door — anyone who enters is saved. The Word names Christ. A first cause is not yet the gospel of Jesus Christ',
      'Teach stays Learn → play → Hold. Arcade juice and One more loops stay. No new quiz chrome',
      'Night Watch and Town stay hidden on Easy. Journal ungated. Core Easy 39 facts stay free. Never a hard paywall',
      'iOS + Android dual path 1.4.79 stays. Pages still cannot charge. Place titles stay Story Creek, Witness Square, Sky Watch, Why Gate, Meaning Ridge',
    ],
  ...
  },
  {
    ...
    items: [
      ...
      'Jericho road teaches mercy: find the hurt man, Help him, take him to the Inn — not Oil / Cloth boxes. Peek-scroll is a look, not a Miss. Only a tap on the open road or a swipe from the road steps. Help stays on a phone-width board',
      'The father runs is a timing dash: sit-on-the-button stalls. Hold to run, let go when it glows, press again — mercy before the hired-hand speech is done. HUGGED! and One more run stay',
      'Settings → Developer (buried like Reset) one-tap jumps every Easy mini-game: road-maze, father-run, gem Match / word search, claim-merge, source-dig, Hold Why Blast — each labeled lesson id + short name. Not on Easy home',
      'Night Watch and Town stay hidden on Easy. Journal ungated. Core Easy 39 facts stay free. Never a hard paywall',
...
export function latestChange(version: string): ChangeNote {
  ...
  if (note.version === '1.4.92') {
    const prior = latestChange('1.4.91')
    return { ...note, items: [...note.items, ...prior.items] }
  }
  if (note.version === '1.4.91') {
    const prior = latestChange('1.4.90')
    return { ...note, items: [...note.items, ...prior.items] }
  }
