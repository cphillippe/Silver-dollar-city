...
assert.match(dailySrc, /A multiverse is a peer to design/)
...
assert.match(obSrc, /Three replies get named: necessity, chance \(often a multiverse\), or design/)
assert.match(obSrc, /pure assertion, no evidence, not a peer to design/)
assert.match(obSrc, /constants are not necessary in that way/)
...
assert.match(evidenceSrc, /A multiverse is a peer explanation with the same evidence as design/)
assert.match(evidenceSrc, /The constants are necessary; they could not have been otherwise/)
