...
assert.equal(APP_VERSION, '1.4.161')
...
assert.match(latestChange(APP_VERSION).items.join('\n'), /PlainTalk|miss teach|#207|Lock In|Main idea|Why · From|Try again/i)
{
...
assert.match(latestChange(APP_VERSION).title, /Lock In|miss teach|PlainTalk/i)
assert.match(latestChange(APP_VERSION).items.join('\n'), /PlainTalk|miss teach|#207|Lock In|Main idea|Why · From|Try again/i)
